...
if (!process.env.RTLS_API_KEY || !process.env.APP_NAMESPACE) {
  console.error('Error: RTLS_API_KEY and APP_NAMESPACE environment variables are required');
  console.error('\nUsage:');
  console.error('  RTLS_API_KEY=your-key APP_NAMESPACE=your-ns npx ts-node examples/websocket-subscriber.ts');
  process.exit(1);
}
